...
This bundle now carries an ad hoc signature, which is enough to run it on
this machine but is not Developer ID signing or notarization. The official
release downloaded from GitHub is signed and notarized and does not need
any of what follows.

One more step on macOS Sequoia and later, for this ad hoc signed bundle only.
